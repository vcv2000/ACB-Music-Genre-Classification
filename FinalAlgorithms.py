...
X_train = pd.read_csv("Usable_X_train.csv").iloc[:,1:]
X_test = pd.read_csv("Usable_X_test.csv").iloc[:,1:]
...
for i in classes:
    ...
    rho = sklearn.metrics.accuracy_score(Y_test_rock, test_y_prediction)
    values.append([i,rho])
